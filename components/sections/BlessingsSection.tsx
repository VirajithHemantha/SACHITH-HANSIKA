      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        >
          <motion.div
            ...
            className="relative mx-auto mb-8 h-32 w-32 rounded-full border-8 border-background bg-background p-[2px] shadow-md shadow-foreground/20 md:h-44 md:w-44"
          >
            <div className="relative flex h-full w-full items-center justify-center rounded-full bg-[radial-gradient(circle,rgba(244,207,171,0.74),rgba(255,255,255,0.95))]">
              <Heart className="h-12 w-12 text-primary fill-[#be7752]/20" />
            </div>
            <Sparkles className="absolute -right-4 -top-2 h-8 w-8 animate-pulse text-primary" />
            <Sparkles className="absolute -bottom-4 -left-2 h-6 w-6 animate-pulse text-primary" />
          </motion.div>
          ...
          >
            <Heart className="h-5 w-5 text-primary fill-[#bf7752]/20" />
            <span className="text-sm font-bold uppercase tracking-[0.2em] text-primary">
              Shared With Love
            </span>
          </motion.div>

          <h2 className="font-serif text-5xl font-medium tracking-tight text-primary md:text-7xl">
            Blessings &{' '}
          ...
          <p className="mx-auto mt-8 max-w-xl text-lg leading-relaxed text-primary">
            Share your heartfelt blessings for our new life together.
          </p>
        </motion.div>
        ...
        <div className="grid items-start gap-8 md:gap-10 lg:grid-cols-[1fr_1.2fr] lg:gap-12">
          ...
          >
            <div className="relative overflow-hidden rounded-[2.5rem] border border-secondary bg-[linear-gradient(150deg,rgba(255,255,255,0.86)_0%,rgba(255,244,228,0.8)_100%)] p-6 shadow-md shadow-foreground/20 backdrop-blur-xl md:p-10">
              <div className="absolute -right-[10%] -top-[10%] h-[150px] w-[150px] rounded-full bg-secondary/30 blur-[40px]" />
              <div className="absolute -bottom-[10%] -left-[10%] h-[150px] w-[150px] rounded-full bg-secondary/28 blur-[40px]" />

              <div className="relative z-10">
                <div className="mb-8 flex items-center gap-3">
                  <span className="flex h-10 w-10 items-center justify-center rounded-full bg-secondary text-background shadow-md">
                    <Stars className="h-5 w-5" />
                  </span>
                  <h3 className="font-serif text-3xl font-medium text-primary">Leave a Message</h3>
                </div>
                ...
                <motion.form onSubmit={handleAddBlessing} className="space-y-6">
                  <label className="group block">
                    ...
                    <div className="relative">
                      <UserRound className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-primary transition-colors group-focus-within:text-primary" />
                      <input
                        type="text"
                        value={visitorName}
                  ...
                  <label className="group block">
                    ...
                    <div className="relative">
                      <PenLine className="pointer-events-none absolute left-4 top-4 h-5 w-5 text-primary transition-colors group-focus-within:text-primary" />
                      <textarea
                        value={newBlessing}
                        onChange={(e) => setNewBlessing(e.target.value)}
                        placeholder="Share your sweetest wishes..."
                        rows={4}
                        className="w-full resize-none rounded-[1.5rem] border border-secondary bg-background/65 px-4 py-4 pl-12 text-primary placeholder-[#d9b89e] outline-none transition-all duration-300 focus:border-secondary focus:bg-background focus:shadow-md shadow-foreground/20 group-hover:bg-background/90"
                        required
                      />
                    </div>
                  </label>
                  ...
                  <motion.button
                    ...
                    className="group relative inline-flex w-full items-center justify-center gap-3 overflow-hidden rounded-full border border-secondary bg-secondary px-6 py-4 text-background shadow-md shadow-foreground/20 transition-all hover:bg-secondary hover:shadow-md shadow-foreground/20"
                  >
                    ...
                    <div className="absolute inset-0 z-0 flex h-full w-full justify-center [transform:skew(-12deg)_translateX(-150%)] group-hover:duration-1000 group-hover:[transform:skew(-12deg)_translateX(150%)]">
                      <div className="relative h-full w-12 bg-background/30" />
                    </div>
                  </motion.button>

                  {submitError && (
                    <p className="text-center text-sm font-medium text-primary">{submitError}</p>
                  )}
                  ...
                  <AnimatePresence>
                    {submitted && (
                      <motion.div
                        ...
                        className="overflow-hidden rounded-2xl border border-secondary/20 bg-secondary/10 px-4 py-3 text-center"
                      >
            ...
            <AnimatePresence mode="popLayout">
              {blessings.map((blessing, index) => (
                <motion.div
                  ...
                  className="group relative overflow-hidden rounded-[2rem] border border-secondary bg-background/70 p-6 shadow-sm backdrop-blur-md transition-all hover:bg-background hover:shadow-md shadow-foreground/20 md:p-8"
                >
                  ...
                  <div className="relative z-10">
                    <p className="mb-4 text-lg font-light leading-relaxed text-primary">"{blessing.message}"</p>

                    <div className="mt-6 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-[#e3bf9a] to-[#bf7752] text-background shadow-sm">
                          <span className="font-serif font-bold tracking-widest">{blessing.name.charAt(0)}</span>
                        </div>
                        <div>
                          <h4 className="font-serif text-lg font-medium text-primary">{blessing.name}</h4>
                          <p className="text-[10px] font-bold uppercase tracking-[0.1em] text-primary">{blessing.timestamp}</p>
                        </div>
                      </div>

                      <motion.div whileHover={{ scale: 1.2, rotate: 5 }} className="cursor-pointer text-primary hover:text-primary">
